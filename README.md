...
By default, Obsidian "scrambles" `.canvas` the z-index array on every save—nodes appear in random creation order, losing spatial and structural meaning. This plugin reads the canvas as a **visual language**, encoding four visual dimensions into stable, deterministic JSON:
