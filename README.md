...
By default, Obsidian "scrambles" the z-index array on every save, losing the spatial and structural meaning that Canvas format is designed to capture. This plugin recompiles the canvas, reencoding visual intent into stable, deterministic JSON.  It interprets the canvas as a visual language, translating four visual dimensions into linear structure:
